...
int initializeWatershed(pointcloud_t* pc) {
	...
	for (int i = 0; i < points->size; i++) {
		...
		if (p->relitiveX != 0) {
			int tempx = p->relitiveY + ((p->relitiveX-1)*width);
			p->north = listGet(points, tempx - 1);
		}if (p->relitiveX != pc->rows - 1) {
			int tempx = p->relitiveY + ((p->relitiveX+1)*width);
			p->south = listGet(points, tempx );
		}if (p->relitiveY != pc->cols-1) {
			int tempy = p->relitiveY + (p->relitiveX * width);
			p->east = listGet(points, tempy+1);
		}if (p->relitiveY != 0) {
			int tempy = p->relitiveY + ((p->relitiveX) * width);
			p->west = listGet(points, tempy-1);
		}
	}
	return 0;
...
void watershedStep(pointcloud_t* pc) {
	...
	double* temps[size];
	for (int i = 0; i < points->size; i++) {
		printf("numbers of loop id 1\n");
		pcd_t* p = listGet(points, i);
		printf("numbers of loop id 2\n");
		pcd_t* east = p->east;
		printf("numbers of loop id 3\n");
		pcd_t* west = p->west;
		printf("numbers of loop id 4\n");
		pcd_t* north = p->north;
